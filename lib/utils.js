...
export function* skipBigram(
  tokens: Array<string>,
  gapLength: number = 2
): Generator<Array<string>, void, boolean> {
  if (gapLength < 1) {
  ...
  for (let idx = 0; idx < (tokens.length - gapLength); idx++) {
    yield [tokens[idx], tokens[idx + gapLength]];
  }
}
 ...
 *
 * @method intersection
 * @param  {Iterator<T>}     a [description]
 * @param  {Iterator<T>}     b [description]
 * @return {Array<T>}          [description]
 */
export function intersection(a: Iterator<string | number>, b: Iterator<string | number>): Array<string | number> {
 ...
 * @return {Array<T>}   [description]
 */
export function lcs(a: Array<string | number>, b: Array<string | number>): Array<string | number> {
  let start = [];
  let end = [];
